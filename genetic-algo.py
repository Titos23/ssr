...
players = range(1, n + 1)
rounds = range(1, n)
ratings = {i: 2000 - 100 * (i - 1) for i in players}
...
H = {i: LpVariable(f"H_{i}", lowBound=0) for i in players}
...
for i in players:
    prob += H[i] == lpSum(h[i, j, r] * ratings[j] for j in players if j != i for r in rounds)
    prob += N[i] == lpSum(h[i, j, r] for j in players if j != i for r in rounds)

# Objective: Minimize breaks and home game variance (avoid quadratic variance)
...
mean_H = lpSum(H[i] for i in players) / n
dev_H = {i: LpVariable(f"dev_H_{i}", lowBound=0) for i in players}
for i in players:
    prob += dev_H[i] >= H[i] - mean_H
    prob += dev_H[i] >= mean_H - H[i]

# Composite objective (equal weights, normalized)
max_dev_H = 1000000
max_breaks = n * (n - 2)
max_dev_N = n
prob += (1/3) * (lpSum(dev_H[i] for i in players) / max_dev_H) + \
        (1/3) * (total_breaks / max_breaks) + \
        (1/3) * (lpSum(dev_N[i] for i in players) / max_dev_N)
